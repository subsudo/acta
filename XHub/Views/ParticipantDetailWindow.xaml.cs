...
public partial class ParticipantDetailWindow : Window
{
    ...
    public void UpdateParticipant(ParticipantIndexEntry? participant, IReadOnlyList<DetailModuleConfig> modules)
    {
        ...
        Title = participant is null ? "Teilnehmerdetails" : $"{participant.DisplayName} - XHub";
        RebuildActions();
        RebuildModules();
    }
    ...
    {
        if (!Directory.Exists(entry.FolderPath)) { MessageBox.Show("Der Teilnehmerordner ist nicht erreichbar.", "XHub", MessageBoxButton.OK, MessageBoxImage.Warning); return; }
    ...
    {
        try { action(ResolveDocumentPath(entry)); }
        catch (Exception ex) { AppLogger.Error($"XHub.DetailWindow.WordAction '{entry.DisplayName}'", ex); MessageBox.Show(ex.Message, "XHub", MessageBoxButton.OK, MessageBoxImage.Error); }
    }
