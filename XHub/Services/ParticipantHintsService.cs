...
public sealed class ParticipantHintsService
{
    ...
    public IReadOnlyDictionary<string, IReadOnlyList<ParticipantHintDisplay>> LoadActiveDisplaysByDocumentPath(IEnumerable<string> documentPaths)
    {
        ...
        ParticipantHintDocument document;
        try
        {
        ...
        {
            AppLogger.Warn($"Hinweise konnten nicht gebündelt geladen werden. Path='{_storePath}', Error='{ex.Message}'");
            return result;
        }

        foreach (var path in paths)
        {
            if (!TryCanonicalizeDocumentPath(path, out var canonicalPath, out _))
            {
    ...
    private static string? TryResolveMappedDriveRoot(string driveName)
    {
        ...
        {
            AppLogger.Info($"Hinweise: WNetGetConnection konnte Laufwerk '{driveName}' nicht aufloesen. Result={result}.");
            return null;
        }
        ...
        AppLogger.Info($"Hinweise: WNetGetConnection konnte Laufwerk '{driveName}' auch mit erweitertem Buffer nicht aufloesen. Result={result}.");
        return null;
    }
