...
public sealed class DocxHeaderMetadataService
{
    ...
    private static readonly Regex InlineCounselorRegex = new(@"Beratungsperson\s*:?\s*(?<code>[\p{Lu}\p{N}]{1,8})\b", RegexOptions.Compiled | RegexOptions.IgnoreCase);
    ...
    private static string? NormalizeCounselorToken(string? value)
    {
        ...
        if (!UpperTokenRegex.IsMatch(cleaned))
        {
            return null;
        }
