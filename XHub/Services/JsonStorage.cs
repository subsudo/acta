...
public static class JsonStorage
{
    ...
    public static T LoadRequired<T>(string path)
    {
        if (!File.Exists(path))
        {
        ...
        try
        {
            return DeserializeFromFile<T>(path)
                   ?? throw new InvalidDataException($"Die Datei '{path}' enthaelt keine gueltigen Daten.");
        }
        catch (Exception ex)
        {
            AppLogger.Warn($"JSON-Laden fehlgeschlagen '{path}': {ex.Message}");
            throw new InvalidDataException($"Die Datei '{path}' ist kein gueltiger XHub-Export.", ex);
        }
    }
    ...
    private static IEnumerable<string> GetTempPathCandidates(string targetPath)
    {
        ...
        yield return Path.Combine(Path.GetTempPath(), $"XHub.{targetFileName}.{guid}.tmp");
    }
