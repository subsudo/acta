...
public class ParticipantSearchService
{
    public IReadOnlyList<ParticipantIndexEntry> Search(string query, IEnumerable<ParticipantIndexEntry> entries, int maxResults = 40)
    {
        ...
        return entries
            .Select(entry => new { Entry = entry, Score = Score(trimmedQuery, queryTokens, fallbackQueryTokens, entry) })
    ...
    {
        var normalizedDisplay = entry.DisplayName.ToLowerInvariant();
        var normalizedQuery = rawQuery.ToLowerInvariant();
        var bestScore = 0;

        if (normalizedDisplay.StartsWith(normalizedQuery, StringComparison.OrdinalIgnoreCase))
        {
