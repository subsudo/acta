...
public class ParticipantIndexService
{
    ...
    private IndexBuildResult BuildIndex(CancellationToken cancellationToken)
    {
        ...
        CollectDirectories(lvBasePath, "LV", directories, warnings);
        if (_config.UseSecondaryServerBasePath)
        {
            CollectDirectories(_config.SecondaryServerBasePath, "LV", directories, warnings);
        }

        CollectDirectories(_config.LbBasePath, "LB", directories, warnings);
        CollectDirectories(_config.StartBasePath, "ST", directories, warnings);
        CollectDirectories(_config.ExitBasePath, "AU", directories, warnings);
    ...
    private static void CollectDirectories(string basePath, string label, List<(string Path, string SourceLabel)> target, List<string> warnings)
    {
        ...
        try
        {
            ...
            {
                foreach (var dateDirectory in Directory.GetDirectories(basePath, "*", SearchOption.TopDirectoryOnly))
                {
                    foreach (var participantDirectory in Directory.GetDirectories(dateDirectory, "*", SearchOption.TopDirectoryOnly))
                    {
                        if (!ShouldIndexDirectory(participantDirectory, label))
                        {
                            continue;
                        }
